...
int main(int argc, char **argv) {
    ...
    if (argc >= 2) {
        ...
        if (strcmp(argv[1], "bridge") == 0 && argc == 7) {
            ...
            printf("\n");
            printf("  变换前:                         变换后:\n");
            printf("  ┌──R1──C──R3──┐                  ┌── R2 ── C ── R3 ──┐\n");
            printf("  │      │      │      Δ→Y    A ── R5 ── O ──                  ├── B\n");
            printf("  A      R5     B    ────►          └── R1 ── D ── R4 ──┘\n");
            printf("  │      │      │\n");
            printf("  └──R2──D──R4──┘  Δ(R12=R5, R51=R2, R52=R1)\n");
            printf("\n");
            DeltaWye dw = delta_to_wye(r1, r2, r5);
            printf("             R5 = "); frac_print(dw.r2); printf("  (↔ 桥上 R5)\n");
            printf("             R2 = "); frac_print(dw.r1); printf("  (↔ 桥上 R2)\n");
            printf("             R1 = "); frac_print(dw.r3); printf("  (↔ 桥上 R1)\n");
